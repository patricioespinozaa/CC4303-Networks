...
# Argumentos
# size tiemout input output host port
size = int(sys.argv[1])
...
lock = threading.Lock()                # Bloqueo para acceso a variables compartidas
cond = threading.Condition(lock)       # Condición para sincronización
ack_received = False                   # variable de confirmación
expected_seq = 0                       # número de secuencia esperado
last_ack = -1                          # último número de secuencia confirmado
eof_received = False                   # EOF recibido
current_packet = b''                   # paquete actual
packet_stats = {                       
    'sent': 0,
    'retransmitted': 0
}
            ...
            with cond:
                if seq == expected_seq:
                    ...
                    try:
                        ...
                        if "HACKEADO" in contenido:
                            print(f"[!] ⚠️ Paquete pirateado detectado en seq {seq}: {contenido}")
